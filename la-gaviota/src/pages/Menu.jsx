...
const Menu = () => {
    ...
    return (
        ...
        <div className="flex items-center justify-between mb-4">
            <h1 className="text-3xl font-menu text-wood-brown text-center w-full -ml-8">
                MENÚ
            </h1>
        </div>

        {/* Categorías (anclas) */}
        <div className="flex gap-3 overflow-x-auto pb-4 sticky bg-wood-brown/60 scrollbar-hide">
            {categories.map((cat) => (
            <a
                key={cat}
                href={`#${cat.replace(/ /g, "-")}`}
                className=" whitespace-nowrap px-4 py-2 rounded-sm font-general text-sm shadow transition-all duration-200 bg-soft-beige text-wood-brown hover:bg-wood-brown hover:text-soft-beige"
            >
                {cat}
            </a>
            ))}
        ...
        {Object.entries(menuData).map(([section, items]) => (
            <section key={section} id={section.replace(/ /g, "-")} className="mt-10">
